...
#     excluded_rows = df_case[df_case.get('sub_afik').isin(EXCLUDED_SUB_AFIK)] if 'sub_afik' in df_case.columns else df_case.iloc[0:0]
...
#     bad_share_by_value = 0.0 if portfolio_value_total == 0 else (bad_value_total / portfolio_value_total)
#     return {
#         'excluded_pct_of_portfolio': excluded_pct_of_portfolio,  # 0..1
#         'portfolio_value_total': portfolio_value_total,
#         'bad_pct_of_portfolio': bad_pct_of_portfolio,            # 0..1
#         'bad_value_total': bad_value_total,
#         'bad_share_by_value': bad_share_by_value,                # 0..1
#     }

# def metric_excluded_bad_pct(df_case_curr: pd.DataFrame, df_case_prev: pd.DataFrame | None) -> tuple[float|None, float|None]:
#     """שינוי סיווג – 'אחוז מתיק האשראי הלא מוחרג' (עפ״י ההנחיה: מחושב על המוחרגים)."""
...
#     """
#     'אחוז מכלל התיק' – סכום pct_of_portfolio_leval רק ל-BAD.
#     נוכחי: Data.xlsx; קודם: DataOld.xlsx (אם יש עמודה רלוונטית).
#     """
#     def calc(df_case: pd.DataFrame) -> float | None:
...
#                        curr_total_bad_pct: float | None,    prev_total_bad_pct: float | None) -> Image.Image:
#     """שקופית 'שינוי סיווג' – מציגה גם 'אחוז מכלל התיק' וגם 'אחוז מתיק האשראי הלא מוחרג' (עפ״י ההנחיה)."""
#     W, H = 1600, 900
...
#     # בלוק סטטיסטיקות (placeholder)
#     def draw_stats(cx, top_y):
#         bw, bh = 560, 230
#         l = cx - bw//2
#         draw_box(l, top_y, l+bw, top_y+bh)
#         pad = 18; y = top_y + pad
...
#             "כניסה לסיווג כבעייתי        --",
#             "יציאה מסיווג כבעייתי        --",
...
#     draw_stats(left_x,  680)
#     draw_stats(right_x, 680)
#     return slide
...
# =========================
def metrics_exec_summary(df_case: pd.DataFrame) -> dict:
    # אין שינויי דיוק פה – משתמשים בערכים שכבר עוגלו בשלב הטעינה
    portfolio_value_total = float(df_case['sec_value'].sum())
...
def metric_bad_entry_exit(df_case_curr: pd.DataFrame | None,            #שינוי סיווג ניירות - כניסה/יציאה לסיווג BAD
    ...
    entries = len(curr_bad - prev_bad)  # נכנסו ל-BAD
    exits   = len(prev_bad - curr_bad)  # יצאו מ-BAD (נשתמש בהמשך)
...
def metric_class_change_count(df_curr, df_prev):            #שינוי סיווג ניירות - מספר שינוי סיווג
    ...
    מטפל בריבוי שורות לאותו sec_id ע"י בחירת label אחד (mode -> first).
    """
...
def main():
    ...
    for case_id in ids:
        ...
        # שינוי סיווג – שני מדדים
        case_prev = df_prev[df_prev['__case'] == case_id].copy() if df_prev is not None else None
        case_prev_prev = df_prev[df_prev['__case'] == case_id].copy() if df_prev is not None else None
        curr_excl_bad_pct, prev_excl_bad_pct = metric_excluded_bad_pct(case_curr, case_prev)
        curr_total_bad_pct, prev_total_bad_pct = metric_total_bad_pct(case_curr, case_prev)
        ...
        white_img = render_white_slide(account_name,
                                       ...
                                       curr_bad_entries, prev_bad_entries,               # כניסה לסיווג בעייתי (ימין=נוכחי; שמאל=-- כי אין PrevPrev)
